...
- [ ] **Library git root enforcement**: When the library is git-tracked, enforce that it is the repository root (not a subdirectory of a larger repo). This simplifies git operations: `git pull` only affects library files, `tome sync` can safely commit without scoping, merge conflicts stay within skill files. Users who want the library inside a dotfiles repo can use a git submodule. Design decision: should `tome init` refuse to `git init` if `library_dir` is already inside a git repo, or warn?
