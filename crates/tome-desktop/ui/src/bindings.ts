...
export const commands = {
	 ...
	 */
	startSync: () => typedError<DesktopSyncOutcome, TomeError>(__TAURI_INVOKE("start_sync")),
	/**
	 ...
	 */
	respondSyncGitConsent: (requestId: string, decision: GitConsentDecision) => typedError<DesktopSyncOutcome, TomeError>(__TAURI_INVOKE("respond_sync_git_consent", { requestId, decision })),
	/**
 ...
 */
export type DesktopSyncOutcome = { kind: "completed"; data: SyncOutcomeWire } | { kind: "git_consent_required"; data: GitConsentRequired };
...
export type GitConsentDecision = "accept" | "decline";
 ...
 */
export type GitConsentRequired = {
	request_id: string,
	stage: GitConsentStage,
	remote_summary: string | null,
	owned_changes: string[],
};

/**  Rust-authoritative stage at which a shared-pool Git action needs consent. */
export type GitConsentStage = 
/**  A fast-forward-only pull may update repository files. */
"pre_pull" | 
/**  A commit/push would publish the listed Tome-owned paths. */
"post_sync";

export type GitHealth = { kind: "available"; clean: boolean; staged: GitChangeSummary; unstaged: GitChangeSummary; upstream: UpstreamState; remote_available: boolean; detached: boolean; operation_in_progress: string | null; latest_commit_age_seconds: number | null } | { kind: "unavailable"; reason: string };
...
export type UpstreamState = { kind: "available"; ahead: number; behind: number; diverged: boolean } | { kind: "unavailable" };
