...
- `tome browse` actions `open` (ViewSource) and `copy path` (CopyPath) now work on Linux — `open` dispatches to `xdg-open` and `copy path` uses the `arboard` crate with both X11 and Wayland (`wayland-data-control`) backends enabled. `open` now uses `.status()` instead of `.spawn()` so a non-zero exit from `xdg-open` (no MIME handler, no DISPLAY) surfaces as `⚠ xdg-open exited N for: <path>` instead of the previous silent `✓ Opened` lie. Both success (`✓`) and failure (`⚠`) outcomes appear in the TUI status bar in place of the keybind line until the next keypress, replacing the prior macOS-only silent-drop behavior. The `sh -c "echo -n ${path} | pbcopy"` invocation is removed (eliminates a command-injection vector). ([#414](https://github.com/MartinP7r/tome/issues/414))
