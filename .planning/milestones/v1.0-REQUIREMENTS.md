...
> **Status: FORWARD-PLANNING DRAFT.** This document scopes a future milestone.
> It is filed under `milestones/` rather than at `.planning/REQUIREMENTS.md`
> so it does not collide with v0.9 planning, which is the next active milestone
> per `PROJECT.md`. Promote this draft to `.planning/REQUIREMENTS.md` when v1.0
> becomes the active milestone (run `/gsd:new-milestone` at that point to
> ratify, fill traceability, and assign GitHub issue numbers).

**Defined:** 2026-04-28
**Status:** Draft — pending `/gsd:new-milestone` ratification when v0.9 ships
...
v0.6–v0.8 hardened the CLI: unified directory model, wizard UX, partial-failure visibility, cross-platform browse actions. v0.9 (planned) closes cross-machine portability via `machine.toml` overrides. v1.0 is the inflection point where tome stops being CLI-only.
...
| **D-GUI-04** | Frontend framework: **TBD in Phase 10 spike** — shortlist React, Solid, Svelte | Decision deferred to spike: React for ecosystem, Solid for performance/size, Svelte for ergonomics. Pick one and commit; no multi-framework UI. |
...
|-------------|-------|--------------|--------|
| CORE-01..05 | Phase 10 | TBD | Pending |
| VIEW-01..06 | Phase 11 | TBD | Pending |
| SYNC-01..05 | Phase 12 | TBD | Pending |
| CFG-01..05 | Phase 13 | TBD | Pending |
| OPS-01..04 | Phase 14 | TBD | Pending |
| BAK-01..04 | Phase 15 | TBD | Pending |
| DIST-01..05 | Phase 16 | TBD | Pending |
...
|-----|--------|------------|
| **v1.0-alpha** | 10 + 11 | Read-only desktop client. Status, list, browse-replacement, doctor. No mutating actions in the GUI; CLI still required for sync/edit. |
| **v1.0-beta** | + 12 + 13 | Sync + config UI. Daily-driver capable for the primary user. |
| **v1.0-rc** | + 14 + 15 | All mutating commands available in the GUI. CLI is functionally optional for steady-state use. |
| **v1.0** | + 16 | Signed, notarized, auto-updating DMG. Public-shippable. |
...
*Requires `/gsd:new-milestone` ratification + traceability fill before phase planning starts.*
