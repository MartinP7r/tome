...
- **Crate-boundary visibility rule (load-bearing — read before writing any test):**
  - `crates/tome/src/*.rs::tests` (child mods inside library modules) have `pub(crate)` access to the `tome` crate. These tests can construct struct literals with `pub(crate)` fields (e.g., `DirectoryConfig { role: Some(role), ... }`) and read `pub(crate)` fields directly (e.g., `config.directories`).
  - `crates/tome/tests/*.rs` integration tests compile as a **SEPARATE crate** — they link against `tome` as an external consumer and can use only `pub` items. `pub(crate)` fields and methods are NOT reachable.
  - **When a `tests/*.rs` test needs to read a `pub(crate)` field, add a `pub` accessor method on the owning type rather than widening the field's visibility.** Accessor methods return `&T` so callers get cheap read-only views while the type retains encapsulation. Plan 05-01 Part D adds three such accessors (`Config::directories()`, `Config::library_dir()`, `Config::exclude()`) for Plan 05-03's integration test.
...
- Code changes: `wizard.rs` (signature + branches + new helper), `lib.rs:160-180` (bail removal + signature update at call site), `config.rs` (three `pub fn` accessor methods per Plan 05-01 Part D — field visibility unchanged), possibly `cli.rs:77-78` (help text).
- Test additions: `wizard.rs::tests` (new), `config.rs::tests` (combo table), `tests/cli.rs` (one new wizard integration test).
- No changes to: `config.rs` non-test code OTHER than the new accessor methods (the wizard tests reach existing API), `discover.rs`, `library.rs`, `distribute.rs`, anything in `browse/`, `manifest.rs`, `lockfile.rs`.
