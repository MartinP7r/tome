...
```rust
        ...
        for dir_type in &ALL_TYPES_FOR_MATRIX {
            ...
            for role in &ALL_ROLES_FOR_MATRIX {
                ...
                if should_pass {
                    ...
                    let err = config.validate().unwrap_err_or_else_panic(format!(
                        "expected INVALID combo ({:?}, {:?}) to fail validate(), but it succeeded",
                        dir_type, role,
                    ));
                    let _ = err; // lint: value is asserted-upon below in the sibling test
                }
            }
        }
    ...
    /// Extension trait: `Result::unwrap_err_or_else_panic` — like unwrap_err but
    /// panics with a custom message (including the Ok value) when the Result is Ok.
    /// Kept inside the test module so we do not add this to the public API.
    trait UnwrapErrOrElsePanic<T, E> {
        fn unwrap_err_or_else_panic(self, msg: String) -> E;
    }
    impl<T: std::fmt::Debug, E> UnwrapErrOrElsePanic<T, E> for std::result::Result<T, E> {
        fn unwrap_err_or_else_panic(self, msg: String) -> E {
            match self {
                Ok(_v) => panic!("{msg}"),
                Err(e) => e,
