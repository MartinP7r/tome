...
- **D-07:** `bindings.ts` is **committed** at `crates/tome-desktop/ui/src/bindings.ts`, with a CI freshness gate (`git diff --exit-code`). **Mechanism corrected during Phase 25 research (2026-05-24):** `build.rs` CANNOT generate it — `build.rs` runs in a separate link unit and cannot see the `#[tauri::command]` fns that `tauri_specta::Builder` needs. Instead: a small **`gen-bindings` binary** in `tome-desktop` shares a `make_builder()` fn with `main.rs` (single source of truth for the command/event registry) and writes the committed `bindings.ts`; CI runs `cargo run -p tome-desktop --bin gen-bindings` then `git diff --exit-code`. `main.rs` also exports under `#[cfg(debug_assertions)]` for the dev loop. Intent (committed file + freshness gate) is unchanged; only the generation entry point moved from `build.rs` → `gen-bindings` bin.

### Owned/Unowned migration (#542, part of CORE-01)
- **D-08:** Replace `SkillEntry::source_name: Option<DirectoryName>` with `ownership: SkillOwnership` where `enum SkillOwnership { Owned { source: DirectoryName }, Unowned { last_owner: Option<DirectoryName> } }`. Lifts the existing `last_directory_name` field into the `Unowned` variant where it belongs. Specta-derives as a TS discriminated union and forces exhaustive handling in the GUI. **Name corrected during Phase 25 research (2026-05-24):** the originally-proposed `SkillProvenance` enum name collides with the existing `discover.rs::SkillProvenance` struct (managed-skill registry/version/git-SHA metadata, used in ~10 sites) — renamed to `SkillOwnership` to disambiguate. **Manifest JSON migration strategy for this field-shape change is a planning detail** — must preserve the existing `#[serde(default)]` round-trip tolerance described in LIB-01..05 / D-10 (research recommends `#[serde(from = ...)]` or migration-on-read to accept both the old `source_name` shape and the new `ownership` enum).
