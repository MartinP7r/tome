...
---
*Last updated: 2026-05-13 — Phase 19 (Doctor/status surface + bugfix bundle) complete. All 8 Phase 19 requirements validated (OBS-06, OBS-07, FIX-01..06). `tome doctor` gains typed substrate (IssueCategory + RepairKind enums with POLISH-04 exhaustiveness sentinels; per-category counts in text summary; `category` field per issue plus `summary.by_category` + `summary.auto_fixable_by_category` maps in JSON; substring-matching anti-pattern eliminated from the dispatcher via exhaustive `match issue.repair_kind` — adding a variant without a handler now fails to compile; closes #530 / FIX-01). `tome status` gains a `Last sync: <RFC-3339>` (or `never`) top-line + per-directory `SKILLS` column; manifest schema lifted with additive `last_synced_at: Option<String>` stamped at `lib.rs:1789` inside the `!dry_run` save block before `manifest::save` per D-LSYNC-3; JSON gains top-level `last_sync` field (OBS-07). Stale "managed symlink(s) tracked in git" check + helper + interactive render path deleted wholesale — v0.10's library-canonical model made the check incapable of firing on clean libraries (#532 / FIX-03). Browse copy-path timing bound relaxed 600ms→2000ms with multi-line FLAKE-FIX comment naming arboard clipboard contention as root cause and rejected clock-injection alternative; 100/100 stability locally (#511 / FIX-02). Backup `push_and_pull_roundtrip` test took Outcome C — flake did not reproduce on M1 macOS (50/50 isolated + 10/10 module + 5/5 lib-suite); defensive FLAKE-WATCH comment with retry-wrapper pseudocode ships in place for future-phase pickup if CI recurs (HARD-14 carry-over). Wizard summary ANSI-width #454 closed administratively via Path 2B — reproduce-first confirmed the existing `tabled[ansi]` feature (commit `0803afb`, April 2026) already produces aligned output; snapshot test `show_directory_summary_aligns_header_with_body_under_ansi` ships as a regression guard (no `strip-ansi-escapes` dep added). Wizard library-default-follows-`tome_home` (#453+#456 / FIX-05) closed via two pinning integration tests in `cli_init.rs` (positive + negative) — implementation at `wizard.rs:670` was already correct; sensitivity verified. `make release VERSION=X.Y.Z` now auto-stamps `## [Unreleased]` → `## [X.Y.Z] - YYYY-MM-DD` via inline `sed -i ''` in the Makefile recipe; CHANGELOG.md staged alongside Cargo.toml/Cargo.lock; idempotent (silent no-op if no `[Unreleased]` section); 3 regression tests in `cli_make_release.rs` (#533 / FIX-06). Test count: 1022 (up from 994 pre-phase per RESEARCH; +28 net new across the 6 substantive plans + 1 verification plan); `make ci` green (fmt-check + clippy `-D warnings` + tests + typos). `[Unreleased]` block intentionally NOT renamed in CHANGELOG — the v0.11 release cut via `make release VERSION=0.11.0` will trigger FIX-06's automation to do the rename. v0.11 milestone is now complete (both phases shipped: Phase 18 OBS-01..05 + Phase 19 OBS-06..07 + FIX-01..06); ready for release cut. v1.0 (Tauri Desktop GUI) remains the next milestone.*

*Last updated: 2026-05-13 — Phase 18 (Observability foundation + sync diagnostics) complete. All 5 OBS-* requirements validated: `tracing` + `tracing-subscriber` substrate wired in `main.rs` via `tracing_init::install()` (OBS-01); `LogLevel::directive()` maps `--verbose`/`--quiet`/default → `debug`/`warn`/`info` with `TOME_LOG` `EnvFilter` override (OBS-02); 6 spans wired in `lib.rs::sync` (top-level `sync` + `discover`/`reconcile`/`consolidate`/`distribute`/`cleanup`) with `time.busy`/`time.idle` on `FmtSpan::CLOSE` (OBS-03); new `change_cause.rs` with `ChangeCause::{HashChanged, PreviouslyFailed, NewlyAdded, DirectoryNowAllowed}` + 9 emission sites in `library.rs`/`distribute.rs` (OBS-04); `SyncReport` extended with `Option<ReconcileReport>` + `render_sync_report` emits `reconcile: ✓ N match · ⚠ M drift · ⚠ K vanished · ⚠ L missing-from-machine` above per-bucket cleanup summary (OBS-05). 24 `eprintln!` warning/info sites migrated to `tracing::{warn,info,debug}!`; wizard prompts, TUI browse output, and user-facing summary tables intentionally preserved on direct stdout per scope discipline. Byte-identical stdout commitment honoured — `cli_status`/`cli_list`/`cli_doctor`/`cli_init` snapshot suites pass with zero diffs. New regression test `sync_verbose_emits_step_spans_on_stderr` pins OBS-03. Two deferrals: `ChangeCause::PreviouslyFailed` emission deferred to v0.12 (requires schema bump for failure persistence — vocabulary reserved); `DirectoryNowAllowed` false-positive on fresh skills accepted and documented in CHANGELOG. CHANGELOG v0.11 work captured under `[Unreleased]` per Keep-a-Changelog convention (Phase 19 release-cut will rename to `[0.11.0]`). Test count: 808 unit + 44 cli_sync (+1 new) + all other suites green. Two human verification items routed (Linux runtime smoke; OBS-05 visual ordering with configured Claude adapter) — non-blocking, documented in `18-VERIFICATION.md`. Phase 19 (Doctor/status surface + bugfix bundle) is next.*
