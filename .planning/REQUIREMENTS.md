...
- [ ] **WHARD-04**: Pure (non-interactive) wizard helpers have unit test coverage: `find_known_directories_in`, registry lookup (`KNOWN_DIRECTORIES`), `DirectoryType::default_role`, and config assembly from a set of selected directories.
- [ ] **WHARD-05**: Integration test that runs the wizard in `--dry-run` mode with `--no-input` and asserts the generated config passes `Config::validate()` and round-trips through TOML without changes.
